...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--temperature', dest='Temperature', nargs='+', type=float, default=[0.0,25.0,50.0,75.0,100.0], help='Array of temperatures [K]')
    parser.add_argument('-b', '--pressure', dest='Pressure', default='1', help='Pressure (atm)')
    parser.add_argument('-c', '--method', dest='Method', default='HA', help='type of lattice dynamics to perform', choices=['HA','SiQ','SiQg','GiQ','GiQg','GaQ','GaQg'])
    parser.add_argument('-d', '--output', dest='Output', default='out', help='Output string')
    parser.add_argument('-e', '--program', dest='Program', default='Tinker', help='method to use to calculate wavenumbers', choices=['Tinker','Test'])
    parser.add_argument('-g', '--coordfile', dest='Coordinate_file', default='molecule.xyz', type=file, help='coordinate file for crystal (in the format the program accepts)')
    parser.add_argument('-i', '--parameter file', dest='Parameter_file', default='keyfile.key', type=file, help='parameter file for crystal (in the format the program accepts)')
    parser.add_argument('-j', '--molecules_in_coord', dest='molecules_in_coord', default=4, type=int, help='number of molecules in the coordinate file')
    parser.add_argument('-k', '--properties', dest='properties_to_save', nargs='+', default=['G','T'], help='calculated properties to store')
    parser.add_argument('-l', '--integration_method', dest='NumAnalysis_method', default='RK4', choices=['RK4'], help='choice of method for temperature gradient integration')
    parser.add_argument('-m', '--integration_step', dest='NumAnalysis_step', default=25.0, type=float, help='step size for temperature gradient integration')
    parser.add_argument('-n', '--gradient_tdelta', dest='LocGrd_Temp_step', default=0.01, type=float, help='steps size for numerical calculation of gradient in T (in K)')
    #MRS: seems like volume and box lattice fraction parameters should be combined?
    parser.add_argument('-o', '--gradient_vdelta', dest='LocGrd_Vol_FracStep', default=3e-02, type=float, help='step size fnumerical calculation of gradient in volume fraction (in fraction)')
    parser.add_argument('-p', '--gradient_boxdelta', dest='LocGrd_LatParam_FracStep', default=5e-05, type=float, help='steps size for box lattice parameter fraction (in fraction)')
    parser.add_argument('-q', '--step_volfrac', dest='StepWise_Vol_StepFrac', default=1.5e-3, type=float, help='step size for stepwise volume change')
    parser.add_argument('-r', '--step_volmin', dest='StepWise_Vol_LowerFrac', default=0.97, type=float, help='minimum relative change for stepwise volume change')
    parser.add_argument('-s', '--step_volmax', dest='StepWise_Vol_UpperFrac', default=1.16, type=float, help='maximum relative change for stepwise volume change' )
    parser.add_argument('-t', '--statistics', dest='Statistical_Mechanics', default='Classical', choices=['Quantum','Classical'], help='type of statistics to use for calculating the lattice dynamics')
    parser.add_argument('-u', '--gruneisen_vol_step', dest='Gruneisen_Vol_FracStep', type=float, default=1.5e-3, help='Gruneisen volume fraction change')
