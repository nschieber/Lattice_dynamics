...
###Additionally, it uses a Gruneisen parameter to predict the changes in wavenumbers with Volume
...
for i in range(len(T_ex)):
  ...
  if i == 0:
    ###Finiding the Gruneisen parameter at the first Temperature point given
    print "Calculating gruneisen parameter"
    if os.path.isfile('wave/iso_p%s_gref.npy'%(ply)) != True:
      ###Using wavenumbers already calculated
      np.save('wave/iso_p%s_gref.npy'%(ply),prop.eig_wvn('temp.xyz',key)[1])
    wvn_ref = np.load('wave/iso_p%s_gref.npy'%(ply))

    if os.path.isfile('') == True:
      os.system('cp   ./temp_exp.xyz')
    else:
      gexp_i = PV[2]*0.0005
      Qs.expand('temp.xyz',key,gexp_i[0],gexp_i[1],gexp_i[2],0.0,0.0,0.0,nmol,ply)
      os.system('mv p%s_temp.xyz temp_exp.xyz'%(ply))
      os.system('cp temp_exp.xyz cords/p%s_gexp.xyz'%(ply))

    if os.path.isfile('wave/iso_p%s_gexp.npy'%(ply)) != True:
      ###Using wavenumbers already calculated
      np.save('wave/iso_p%s_gexp.npy'%(ply),prop.eig_wvn('temp_exp.xyz',key)[1])
    wvn_exp = np.load('wave/iso_p%s_gexp.npy'%(ply))

    PV_exp = prop.PV('temp_exp.xyz',prs)
    V_ref = PV[0]
    gru = prop.Gru_param(wvn_ref,V_ref,wvn_exp,PV_exp[0])
    os.system('rm temp_exp.xyz')

  if T_ex[i] != T_ex[-1]:
    if Dv[i] == 0.0:
      print "Determining dV/dT at T= %sK"%(T_ex[i])
      K = np.zeros(4)
      h = T_ex[i+1] - T_ex[i]
      ...
      for j in range(len(K)):
        ...
        K[j] = Qs.iso_grad_gru(RK_cord,key,prs,ply,nmol,T_ex[i]+t[j],path+'_k%s-%s'%(j+1,T_ex[i]),gru,wvn_ref,V_ref)
  ...
  wvn = Qs.wvn_gru(wvn_ref,V_ref,PV[0],gru)
